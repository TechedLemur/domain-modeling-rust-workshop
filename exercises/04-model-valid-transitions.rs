// Exercise 4: Model valid transitions
//
// Each state now has its own type, with methods for the allowed transitions.
//
// Tasks:
...
impl CreatedOrder {
    fn pay(self, payment_id: String) -> PaidOrder {
        todo!("Return a PaidOrder with the same ID and the supplied payment ID")
    }

    fn cancel(self) -> CancelledOrder {
        todo!("Return a CancelledOrder with the same ID")
    }
}

impl PaidOrder {
    fn ship(self, tracking_number: String) -> ShippedOrder {
        todo!("Preserve the order and payment IDs and add the tracking number")
    }
}
...
fn describe(order: &Order) -> String {
    match order {
        ...
        // TODO: Replace this placeholder with the two missing variant patterns.
        _ => todo!("Describe shipped and cancelled orders"),
    }
}
